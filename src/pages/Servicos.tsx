...
export default function Servicos() {
    ...
    const [formData, setFormData] = useState<{
        codigo: string; nome: string; categoriaId: string; unidade: string; precoBase: number; precoMinimo: number; atribuicaoId: string; moedaId: string; ativo: boolean;
    }>({
        codigo: '', nome: '', categoriaId: '', unidade: 'HR', precoBase: 0, precoMinimo: 0, atribuicaoId: '', moedaId: '', ativo: true
    });
    ...
    const fetchItens = useCallback(async () => {
        try {
            ...
            setItens(res.data.filter((i: ItemCatalogo) => i.tipo === TipoItemCatalogo.Servico));
        } catch (err) {
            console.error(err);
        } finally {
            setLoading(false);
    ...
    const handleOpenModal = (item?: ItemCatalogo) => {
        if (item) {
            ...
            setFormData({
                ...
                atribuicaoId: item.atribuicaoId || '', moedaId: item.moedaId || '', ativo: item.ativo
            });
        } else {
            setEditingId(null);
            const defaultMoedaId = moedas.length > 0 ? String(moedas[0].value) : '';
            setFormData({ codigo: '', nome: '', categoriaId: '', unidade: 'HR', precoBase: 0, precoMinimo: 0, atribuicaoId: '', moedaId: defaultMoedaId, ativo: true });
        }
        setFormErrors({});
        setShowModal(true);
    };
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            // Pre-configure service settings
            const payload = {
                ...formData,
                tipo: TipoItemCatalogo.Servico,
                vendavel: true,
                compravel: false,
                estocavel: false,
                serializado: false,
                categoriaId: formData.categoriaId || null,
                atribuicaoId: formData.atribuicaoId || null,
